...
const digraph = name => body => `digraph ${name ? `${name} ` : ''}{ node [shape=record]; ${body}; }`;
const subgraph = name => body => `subgraph ${name ? `cluster_${name} ` : ''}{ ${body}; }`;
const edge = (a, b) => `${a} -> ${b};`;
...
const emit = pipe(dots, digraph('root'));
